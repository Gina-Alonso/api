...
public record DatosRegistroMedico(
        ...
        @NotBlank(message = "{documento.obligatorio}") @Pattern(regexp = "\\d{7,9}",  message = "{documento.invalido}") String documento,
        @NotNull(message = "{especialidad.obligatorio}") Especialidad especialidad,
        @NotNull(message = "{direccion.obligatorio}") @Valid DatoDireccion direccion
